...
function ArkInventory.OnInitialize( )
	...
	for item, spell in pairs( ArkInventory.Const.CompanionTranslationData ) do
		...
		if type( spell.id ) == "number" then
			...
			ArkInventory.Const.CompanionData[spell.id]["sg"] = spell.sg
			ArkInventory.Const.CompanionData[spell.id]["sf"] = spell.sf
			ArkInventory.Const.CompanionData[spell.id]["sw"] = spell.sw
...
function ArkInventory.UiTabToNext( frame, w_curr, w_prev, w_next )

	if not frame or not w_curr or not w_prev or not w_next then
		return
	end
