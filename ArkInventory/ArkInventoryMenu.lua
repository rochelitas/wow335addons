					...
					if numTokenTypes > 0 then
						...
						for j = 1, numTokenTypes do
							...
							if isHeader then
								...
								local checked = ArkInventory.db.char.option.ldb.currency.tracked[name]
								
								local t1 = name
								local t2 = ArkInventory.Localise["LDB_CURRENCY_SELECT"]
								if checked then
									t1 = "|cff00ff00" .. name
									t2 = ArkInventory.Localise["LDB_CURRENCY_DESELECT"]
								end
...
--[[
										...
										ArkInventory.db.char.option.ldb.currency.tracked[name] = not ArkInventory.db.char.option.ldb.currency.tracked[name]
		...
		ArkInventory.Lib.DewDrop:Open( this,
			...
			"children", function( level, value )
				...
				if level == 2 then
					...
					local track = ArkInventory.db.char.option.ldb.mounts[value].track
					
					if value == "flying" then
						header = ArkInventory.Localise["LDB_MOUNTS_FLYING"]
					elseif value == "water" then
						header = ArkInventory.Localise["LDB_MOUNTS_WATER"]
					end
					...
					for k = 1, n do
						...
						local ok = false
						if cd and ( not cd.r ) then
						
							if ( not ok ) and value == "ground" and cd.sg then
								ok = true
								speed = cd.sg
							end
							
							if ( not ok ) and value == "flying" and cd.sf then
								ok = true
								speed = cd.sf
							end
							
							if ( not ok ) and value == "water" and cd.sw then
								ok = true
								speed = cd.sw
							end
						...
						if ok then
							...
							creatureName = string.format( "%s [%s%%] ", creatureName, speed )
							...
							ArkInventory.Lib.DewDrop:AddLine(
								...
								"func", function( )
									...
									if ArkInventory.db.char.option.ldb.mounts[value].track == creatureSpellID then
										ArkInventory.db.char.option.ldb.mounts[value].track = nil
									else
										ArkInventory.db.char.option.ldb.mounts[value].track = creatureSpellID
									end
					...
					if cc > 1 then
						...
						if not track then
							...
							local slider_value = ArkInventory.db.char.option.ldb.mounts[value].min
							...
							ArkInventory.Lib.DewDrop:AddLine(
								...
								"sliderStep", 10,
								"sliderFunc", function( v )
									ArkInventory.db.char.option.ldb.mounts[value].min = v
								end	
							)
