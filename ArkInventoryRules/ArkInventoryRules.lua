...
function ArkInventoryRules.System.soulbound( )
	...
	if ArkInventoryRules.Item.sb then
		return true
	end
...
function ArkInventoryRules.System.id( ... )
	...
	for ax = 1, ac do
		...
		local e = string.sub( ArkInventory.ObjectIDInternal( ArkInventoryRules.Item.h ) .. ":", 1, string.len( arg ) )
...
function ArkInventoryRules.System.type( ... )
	...
	local e = string.lower( select( 6, GetItemInfo( ArkInventoryRules.Item.h ) ) or "" )
...
function ArkInventoryRules.System.subtype( ... )
	...
	local e = string.lower( select( 7, GetItemInfo( ArkInventoryRules.Item.h ) ) or "" )
...
function ArkInventoryRules.System.equip( ... )
	...
	local e = strtrim( select( 9, GetItemInfo( ArkInventoryRules.Item.h ) ) or "" )
...
function ArkInventoryRules.System.name( ... )
	...
	if not ArkInventoryRules.Item.h then
		return false
	end
	
	local e = string.lower( select( 3, ArkInventory.ObjectInfo( ArkInventoryRules.Item.h ) ) or "" )
...
function ArkInventoryRules.System.quality( ... )
	...
	if not ArkInventoryRules.Item.q then
		return false
	end
...
function ArkInventoryRules.System.itemlevelstat( ... )
	...
	local level = select( 4, GetItemInfo( ArkInventoryRules.Item.h ) ) or 0
...
function ArkInventoryRules.System.itemleveluse( ... )
	...
	local level = select( 5, GetItemInfo( ArkInventoryRules.Item.h ) ) or 0
...
function ArkInventoryRules.System.periodictable( ... )
	...
	for ax = 1, ac do
		...
		if ArkInventory.Lib.PeriodicTable:ItemInSet( ArkInventoryRules.Item.h, strtrim( arg ) ) then
			return true
		end
...
function ArkInventoryRules.System.tooltip( ... )
	...
	if not ArkInventoryRules.Item or not ArkInventoryRules.Item.h or ArkInventoryRules.Item.bag_id == nil or ArkInventoryRules.Item.slot_id == nil or ArkInventoryRules.Item.class ~= "item" then
		return false
	end
	
	if ArkInventoryRules.Item.test_rule then
	
		ArkInventory.TooltipSetHyperlink( ArkInventoryRules.Tooltip, ArkInventoryRules.Item.h )
	
	else
		
		-- tooltip is also used offline mode
		if ArkInventory.Global.Location[ArkInventoryRules.Item.loc_id].isOffline then
			ArkInventory.TooltipSetHyperlink( ArkInventoryRules.Tooltip, ArkInventoryRules.Item.h )
		else
			local bliz_id = ArkInventory.BagID_Blizzard( ArkInventoryRules.Item.loc_id, ArkInventoryRules.Item.bag_id )
			ArkInventory.TooltipSetItem( ArkInventoryRules.Tooltip, bliz_id, ArkInventoryRules.Item.slot_id )
		end
...
function ArkInventoryRules.System.outfit( ... )
	...
	if select( 11, ArkInventory.ObjectInfo( ArkInventoryRules.Item.h ) ) == "" then
		return false
	end
	...
	--elseif IsAddOnLoaded( "ClosetGnome" ) then
		...
		--return ArkInventoryRules.System.outfit_closetgnome( t, ArkInventoryRules.Item.h )
	end
...
function ArkInventoryRules.System.outfit_outfitter( ... )
	...
	if not ArkInventoryRules.Item.h or not IsAddOnLoaded( "Outfitter" ) or not Outfitter_IsInitialized( ) then
		return false
	end
	
	local ItemInfo = Outfitter_GetItemInfoFromLink( ArkInventoryRules.Item.h )
...
function ArkInventoryRules.System.outfit_closetgnome( ... )
	...
	if not ArkInventoryRules.Item.h or not IsAddOnLoaded( "ClosetGnome" ) then
		return false
	end
	...
	if ClosetGnome.GetOutfitsUsingItem then
		...
		Outfits = { ClosetGnome:GetOutfitsUsingItem( ArkInventoryRules.Item.h ) }
...
function ArkInventoryRules.System.outfit_itemrack( ... )
	...
	if not ArkInventoryRules.Item.h or not IsAddOnLoaded( "ItemRack" ) then return false end
	
	local id = string.match( ArkInventoryRules.Item.h or "", "item:(.+):%-?%d+" ) or 0
	...
	for setnum = 1, equipsets do 
		...
		for k, location in pairs( set ) do
			...
			if wearing or bank or bags then
				...
				if h == ArkInventoryRules.Item.h then
...
function ArkInventoryRules.System.vendorprice( opt, t )
	...
	local price_per = select( 11, GetItemInfo( ArkInventoryRules.Item.h ) )
	
	if price_per == nil then
	
		--ArkInventory.Output( "item=[", ArkInventoryRules.Item.h, "], nil sell value" )
		return false
		
	elseif price_per > 0 then
	
		local count = ArkInventoryRules.Item.count or 1
		...
		if type( v ) == "number" then
			...
			if opt == 1 then
				...
				--ArkInventory.Output( "[", count, "] x [", ArkInventoryRules.Item.h, "] = [", price, "], under=[", v, "]" )
				...
				--ArkInventory.Output( "[", count, "] x [", ArkInventoryRules.Item.h, "] = [", price, "], over=[", v, "]" )
...
function ArkInventoryRules.System.characterlevelrange( ... )
	...
	if not ArkInventoryRules.Item.h or not ArkInventoryRules.Item.class == "item" then
		return false
	end
	...
	local ilevel = select( 5, GetItemInfo( ArkInventoryRules.Item.h ) ) or clevel
...
function ArkInventoryRules.System.bag( ... )
	...
	for ax = 1, ac do
		...
		if arg == ArkInventoryRules.Item.bag_id then
			return true
		end
...
function ArkInventoryRules.System.location( ... )
	...
	if not ArkInventoryRules.Item.loc_id then
		return false
	end
	...
	for ax = 1, ac do
		...
		if ArkInventoryRules.Item.loc_id == k then
			return true
		end
	end
...
function ArkInventoryRules.System.count( ... )
	...
	if ArkInventoryRules.Item.count >= arg1 then
		return true
	end
...
function ArkInventoryRules.System.stacks( )
	...
	if select( 10, ArkInventory.ObjectInfo( ArkInventoryRules.Item.h ) ) > 1 then
		return true
	end
...
function ArkInventoryRules.System.trash( )
	...
	local id = select( 2, ArkInventory.ObjectStringDecode( ArkInventoryRules.Item.h ) )
	...
	if IsAddOnLoaded( "SellJunk" ) then
		local poor = ( ArkInventoryRules.Item.q == 0 )
		if ( ( poor ) and ( not SellJunk:isException( ArkInventoryRules.Item.h ) ) ) or ( ( not poor ) and ( SellJunk:isException( ArkInventoryRules.Item.h ) ) ) then
			return true
		end
	end
	...
	if ArkInventoryRules.Item.q == 0 then
		return true
	end
...
function ArkInventoryRules.Frame_Rules_Build_Sort( )
...
function ArkInventoryRules.Frame_Rules_Build_Row( )
...
function ArkInventoryRules.Frame_Rules_Build_Table( f )

	if not f or type( f ) ~= "string" then f = this:GetName( ) end

	local p = _G[f]
	if not p then
		ArkInventory.Output( RED_FONT_COLOR_CODE, "OOPS: Invalid value at ArkInventoryRules.Frame_Rules_Build_Table( [", f, "] )" )
		return
	end
...
function ArkInventoryRules.Frame_Rules_Table_Row_OnClick( )
	...
	if IsShiftKeyDown( ) then
		...
		ArkInventoryRules.Frame_Rules_Table_Refresh( )
...
function ArkInventoryRules.Frame_Rules_Table_Refresh( f )
...
function ArkInventoryRules_Frame_Rules_Table_Refresh( )
	ArkInventoryRules.Frame_Rules_Table_Refresh( )
end
...
function ArkInventoryRules.EntryIsValid( rid, data )
	...
	if not data.formula or strtrim( data.formula ) == "" then
		...
		ArkInventoryRules.Item = { test_rule=true, class="item", bag_id=0, slot_id=1, count=1, q=1, sb=true, h="item:6948:0:0:0:0:0:0:0" } -- hearthstone
...
function ArkInventoryRules.Frame_Rules_Button_Modify( t )
...
function ArkInventoryRules.Frame_Rules_Button_Modify_Ok( )
...
function ArkInventoryRules.Frame_Rules_Button_Modify_Cancel( )
...
function ArkInventoryRules.Frame_Rules_Button_View_Add( )
	return ArkInventoryRules.Frame_Rules_Button_Modify( "A" )
end

function ArkInventoryRules.Frame_Rules_Button_View_Edit( )
	return ArkInventoryRules.Frame_Rules_Button_Modify( "E" )
end

function ArkInventoryRules.Frame_Rules_Button_View_Remove( )
	return ArkInventoryRules.Frame_Rules_Button_Modify( "R" )
end
