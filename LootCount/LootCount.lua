...
function LootCount_OnLoad()
	...
	StaticPopupDialogs["LOOT_COUNT_GOAL"] = {
		...
		OnShow = function()
			if (not LootCount_InitValue) then LootCount_InitValue=0; end
			getglobal(this:GetName().."EditBox"):SetText(LootCount_InitValue);
		end,
		OnAccept = function()
			LootCount_ButtonForGoal.goal=tonumber(getglobal(this:GetParent():GetName().."EditBox"):GetText());
...
function LootCount_Quest.ClearItemButton(button)
	...
	getglobal(button:GetName().."IconTexture"):SetTexture(LootCount_Plugin[LootCount_Quest.plugin].sTexture);
end
...
function LootCount_SetItems(button,other,fastbuttons)
	...
	if (button.itemID) then
		...
		if (button.empty) then
			getglobal(button:GetName().."IconTexture"):SetTexture(1,1,1,0);
			return;
		end
				...
				while (not LootCount_GetButton(pos,line).deleted) do
					while (not LootCount_GetButton(pos,line).deleted) do
						...
						if (thisbutton.empty) then
							getglobal(thisbutton:GetName().."IconTexture"):SetTexture("Interface\\Buttons\\UI-Quickslot");
						end
						pos=pos+xdir;
					end
					LootCount_ShowEmpty(pos,line);
...
-- Validate input
	...
	if (msg=="div") then
		LootCount_OnEnter(true);				-- Show empty slots
...
function LootCount_GlobalFromMenu()
	LootCount_Populated.Global=this.value;
...
function LootCount_Poist(button)
	...
	button:Hide();
	getglobal(button:GetName().."OurStock"):Hide();
	getglobal(button:GetName().."MyCount"):Hide();
end
